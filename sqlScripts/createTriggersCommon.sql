...
begin
    ...
    update public.database_stats set current_bytes = new_size where one_id = true;
...
begin
    select * into new_size from public.calc_user_schemas_size();
    update public.database_stats set current_bytes = new_size;
end;
$$ language plpgsql;
...
declare
    ...
    max_size             int;
begin
    select fill_percent into current_fill_percent from public.database_stats;
    select current_bytes into current_size from public.database_stats;
    select max_bytes into max_size from public.database_stats;
...
declare
    ...
    res_size         bigint = 0;
begin
    foreach user_schema_name in array (select user_schema_names from public.database_stats)
        loop
